...
export default function IndividualsIndex() {
  ...
  const columns = useMemo(() => [
    ...
    columnHelper.accessor(
      row => `${row.first_name} ${row.middle_name || ''} ${row.last_name} ${row.second_last_name || ''}`.trim(),
      {
        id: 'full_name',
        header: 'Nombre',
              ...
              return (
                <div key={individual.id} className="bg-white dark:bg-zinc-900 rounded shadow p-3 flex flex-col gap-2">
                  ...
                  >
                    ...
                    <span className={expandedTitles[individual.id] ? '' : 'truncate'}>
            ...
            <div className="flex items-center justify-between">
              <div className="text-xs text-gray-500 whitespace-nowrap">
                <span className="inline-flex items-center">
                  <span className="hidden xs:inline">{individuals?.meta?.from || 0}-{individuals?.meta?.to || 0}</span>
